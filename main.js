...
class GoodChain
{
	constructor ({chain, state, nodes, dbPath, validator}={})
	{
		...
		self.rewardToCharity = 0.2; // The percentage of the block reward that will be sent to the charity
		...
		self.nodePath = __dirname;
		self.keysPath = join(self.nodePath, "keys");
	...
	async init_validator (cValidator = {})
	{
		const self = this;
		let keys;
		if (!cValidator.publicKey || !cValidator.privateKey)
		{
